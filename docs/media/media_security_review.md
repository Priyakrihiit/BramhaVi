# Enterprise DAM Security Review
...
- **Role-Based Access Control (RBAC)**: Checks specific user role permission codes (e.g. `cms:media:write`).
- **Content-Based Access Control (CBAC)**: Checks object-level mappings inside the `MediaPermission` model to authorize specific UUID targets.
